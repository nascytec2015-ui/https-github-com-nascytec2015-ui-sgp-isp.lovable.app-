...
export function useOsEvidencias(): UseOsEvidenciasReturn {
  ...
  const fetchEvidencias = useCallback(
    async (osId: string) => {
      if (!supabase) return;
  ...
  const uploadEvidencia = useCallback(
    async (osId: string, file: File, tipo: string, descricao: string) => {
      ...
      try {
        ...
        // Registrar na tabela
        const { error: dbError } = await supabase.from("os_evidencias").insert([
          {
            os_id: osId,
  ...
  const deleteEvidencia = useCallback(
    async (evidenciaId: string, url: string) => {
      ...
      try {
        ...
        if (filePath) {
          // Deletar do Storage
          await supabase.storage.from("os_evidencias").remove([filePath]);
        }

        // Deletar registro da tabela
        const { error } = await supabase.from("os_evidencias").delete().eq("id", evidenciaId);
