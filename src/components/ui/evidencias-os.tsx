            ...
            {evidencias.map((evi) => (
              <Card key={evi.id} className="p-3 flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <div className="flex-shrink-0">{getIconForType(evi.tipo)}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{evi.descricao}</p>
                    <div className="flex gap-2 text-xs text-muted-foreground">
                      <span className="capitalize bg-muted px-2 py-0.5 rounded">{evi.tipo}</span>
                      {evi.tamanho_bytes && <span>{formatFileSize(evi.tamanho_bytes)}</span>}
                      {evi.created_at && (
